...
public class PageV3 extends Table {

	private ArrayList<JsonValue> data;
	private ArrayList<JsonValue> dataCurrentPage;
	...
	public PageV3(float width, float height) {
		this.setSize(width, height);
		tablePage = new Table();
		tablePage.setTransform(true);
		scrollPage = new ScrollPane(tablePage);
		scrollPage.setTransform(true);
		listItemPage = new ArrayList<PageV3.ItemPage>();
		data = new ArrayList<JsonValue>();
	...
	public void addData(JsonValue data) {
		this.data.add(data);
	}
	...
	public ArrayList<JsonValue> getCurrentDataPage() {
		return dataCurrentPage;
	}

	public ArrayList<JsonValue> getDataFromPage(int i) {
		ArrayList<JsonValue> data = new ArrayList<JsonValue>();
		int begin = (i - 1) * 15;
		int end = begin + 14;
		if (end != 0) {
			for (int j = begin; j <= end; j++) {
				if (this.data.size() > j) {
					JsonValue item = this.data.get(j);
					data.add(item);
				} else {
					break;
				}
