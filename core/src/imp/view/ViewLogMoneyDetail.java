...
public class ViewLogMoneyDetail extends View {
	...
	public ViewLogMoneyDetail buildComponent(int type) {
		...
		Image bgDatetime = new Image(new NinePatch(
				Assets.instance.ui.reg_ninepatch, color_white));
		...
		btnXem.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				if (stateofpartnerFun.equals("1")) {
					...
					Loading.ins.show(ViewLogMoneyDetail.this);
			...
			if (result) {
				...
				if (arr.size > 0) {
					for (int i = 0; i < arr.size; i++) {
						...
						data.add(agencyname);
						data.add(full_name);
						data.add(agency_transfer);
						data.add(agency_transfer_full_name);
						data.add(type + "");
						data.add(str_money_before);
						data.add(str_money_after);
						data.add(str_money_transfer);
						data.add(currency);
						data.add(note);
						data.add(date);
	...
	void loadListDetail() {
		for (int i = 0; i < pages.getCurrentDataPage().size(); i++) {
			...
			itemList.addComponent(
					...
					40, 45);
			itemList.addComponent(
					new Label(data.get(7) + " " + data.get(8), Style.ins
							.getLabelStyle(18, FontType.Regular, Color.WHITE)),
					40, 15);
			Label lbTime = new Label(data.get(11), Style.ins.getLabelStyle(18,
					FontType.Regular, Color.WHITE));
			itemList.addComponent(lbTime,
					itemList.getWidth() - lbTime.getWidth() - 10, 45);

			String titleHT1 = "cấp";
			String titleHT2 = "nhận";
			if (data.get(4).equals("1")) {
				titleHT1 = "nhận";
				titleHT2 = "cấp";
			}
			itemList.addSubItem(new Label("Người " + titleHT1 + " tiền",
			...
			itemList.addSubItem(
					new Label(data.get(0), Style.ins.getLabelStyle(18,
							FontType.Light, Constants.COLOR_ACTIONBAR)),
					itemList.getWidth(), 25);
			itemList.addSubItem(new Label("Người " + titleHT2 + " tiền",
					Style.ins.getLabelStyle(15, FontType.Light, Color.BLACK)),
					itemList.getWidth(), 25);
			itemList.addSubItem(
					new Label(data.get(3), Style.ins.getLabelStyle(18,
							FontType.Light, Constants.COLOR_ACTIONBAR)),
					itemList.getWidth(), 25);
			itemList.addSubItem(
					new Label("Số điện thoại", Style.ins.getLabelStyle(15,
					...
					25);
			itemList.addSubItem(
					new Label(data.get(2), Style.ins.getLabelStyle(18,
							FontType.Light, Constants.COLOR_ACTIONBAR)),
					itemList.getWidth(), 25);
			if (roldId != 0) {
				itemList.addSubItem(
						new Label("Số tiền trước khi " + titleHT1, Style.ins
								.getLabelStyle(15, FontType.Light, Color.BLACK)),
						itemList.getWidth(), 25);
				itemList.addSubItem(
						new Label(data.get(5) + " " + data.get(8), Style.ins
								.getLabelStyle(18, FontType.Light,
										Constants.COLOR_ACTIONBAR)), itemList
								.getWidth(), 25);
				itemList.addSubItem(
						new Label("Số tiền sau khi " + titleHT1, Style.ins
								.getLabelStyle(15, FontType.Light, Color.BLACK)),
						itemList.getWidth(), 25);
				itemList.addSubItem(
						new Label(data.get(6) + " " + data.get(8), Style.ins
								.getLabelStyle(18, FontType.Light,
										Constants.COLOR_ACTIONBAR)), itemList
								.getWidth(), 25);
			}
			itemList.addSubItem(
					new Label("Số tiền " + titleHT1, Style.ins.getLabelStyle(
			...
			itemList.addSubItem(
					...
					25);
			itemList.addSubItem(
					new Label(data.get(10), Style.ins.getLabelStyle(18,
							FontType.Light, Constants.COLOR_ACTIONBAR)),
					itemList.getWidth(), 25);
