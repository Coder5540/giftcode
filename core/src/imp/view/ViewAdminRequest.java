					...
					for (int i = 0; i < content.size; i++) {
						...
						newItem.btSet.addListener(new ClickListener() {
							...
							public void clicked(InputEvent event, float x,
								...
								dia.text("Bạn có chắc chắn thêm tài khoản? Một tin nhắn sẽ được tự động gửi đến số điện thoại: "
										+ phone);
						...
						newItem.btDel.addListener(new ClickListener() {
							...
							public void clicked(InputEvent event, float x,
								...
								final GetActiveCodeListener listener = new GetActiveCodeListener();
								DialogCustom dia = new DialogCustom("");
								dia.text("Bạn có chắc chắn hủy đăng ký của "
										+ phone + " không?");
								dia.button("Ok", new Runnable() {
									@Override
									public void run() {
										Loading.ins.show(ViewAdminRequest.this);
										Request.getInstance().rejectActive(
												phone,
												AppPreference.instance.name,
